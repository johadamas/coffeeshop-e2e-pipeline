...
def project_setup_pipeline():
    ...
    with TaskGroup("create_azure_resource", tooltip="Tasks for Terraform operations") as terraform_group:
        ...
        get_access_key_and_container = BashOperator(
            ...
            CONTAINER_NAME=$(terraform -chdir=/usr/local/airflow/include/terraform output -raw landing_container_name)
            STORAGE_ACCOUNT_NAME=$(terraform -chdir=/usr/local/airflow/include/terraform output -raw storage_account_name)
            airflow variables set ACCESS_KEY $ACCESS_KEY
            airflow variables set CONTAINER_NAME $CONTAINER_NAME
            airflow variables set STORAGE_ACCOUNT_NAME $STORAGE_ACCOUNT_NAME
            """,
        )
    ...
    terraform_group >> file_path >> upload_to_landing_container(file_path)

# DAG instantiation
setup_dag = project_setup_pipeline()
