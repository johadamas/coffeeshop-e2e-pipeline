...
locals {
  notebooks = [
    {
      path      = "${path.module}/../databricks/notebooks/1.storageMount.py",
      dest_path = "/Workspace/Users/${var.user_email}/1.storageMount"
    },
    {
      path      = "${path.module}/../databricks/notebooks/2.ingestBronze.py",
      dest_path = "/Workspace/Users/${var.user_email}/2.ingestBronze"
    },
    {
      path      = "${path.module}/../databricks/notebooks/3.transformSilver.py",
      dest_path = "/Workspace/Users/${var.user_email}/3.transformSilver"
    },
    {
      path      = "${path.module}/../databricks/notebooks/4.transformGold.py",
      dest_path = "/Workspace/Users/${var.user_email}/4.transformGold"
    },
    {
      path      = "${path.module}/../databricks/notebooks/5.loadDelta.py",
      dest_path = "/Workspace/Users/${var.user_email}/5.loadDelta"
    },
    {
      path      = "${path.module}/../databricks/notebooks/6.pipelineTest.py", 
...
resource "databricks_job" "etl_pipeline" {
  ...
  task {
    ...
    run_if   = "ALL_SUCCESS"
    notebook_task {
      notebook_path = databricks_notebook.notebooks["${path.module}/../databricks/notebooks/1.storageMount.py"].path
      source        = "WORKSPACE"
    }
  ...
  task {
    ...
    notebook_task {
      notebook_path = databricks_notebook.notebooks["${path.module}/../databricks/notebooks/2.ingestBronze.py"].path
      source        = "WORKSPACE"
    }
    existing_cluster_id = databricks_cluster.cluster.id
    timeout_seconds     = 0
  ...
  task {
    ...
    notebook_task {
      notebook_path = databricks_notebook.notebooks["${path.module}/../databricks/notebooks/3.transformSilver.py"].path
      source        = "WORKSPACE"
    }
    existing_cluster_id = databricks_cluster.cluster.id
    timeout_seconds     = 0 
  ...
  task {
    ...
    notebook_task {
      notebook_path = databricks_notebook.notebooks["${path.module}/../databricks/notebooks/4.transformGold.py"].path
      source        = "WORKSPACE"
    }
    existing_cluster_id = databricks_cluster.cluster.id
    timeout_seconds     = 0 
  ...
  task {
    ...
    notebook_task {
      notebook_path = databricks_notebook.notebooks["${path.module}/../databricks/notebooks/5.loadDelta.py"].path
      source        = "WORKSPACE"
    }
    existing_cluster_id = databricks_cluster.cluster.id
    timeout_seconds     = 0
